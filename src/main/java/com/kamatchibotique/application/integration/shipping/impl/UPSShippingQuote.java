...
import com.kamatchibotique.application.utils.DataUtils;
	...
	public List<ShippingOption> getShippingQuotes(
		...
		try {
			...
			if(!regions.contains(store.getCountry().getIsoCode())) {
				throw new IntegrationExceptionService("Can't use the service for store country code ");
			}
...
/*			if (result != 200) {
			...
			if (!StringUtils.isBlank(parsed.getErrorCode())) {
					...
					LOGGER.error("Can't process UPS statusCode="
							...
							+ parsed.getError());
				throw new IntegrationExceptionService(parsed.getError());
			}
			...
			if (parsed.getOptions() == null || parsed.getOptions().size() == 0) {
				...
				throw new IntegrationExceptionService("No shipping options available for the configuration");
			}
			...
			Map serviceMap = com.kamatchibotique.application.util.ShippingUtil
				...
				for(ShippingOption option : shippingOptions) {
					...
					if(option.getOptionPrice()==null) {
						...
						if(StringUtils.isBlank(priceText)) {
							throw new IntegrationExceptionService("Price text is null for option " + name);
						}
						try {
							BigDecimal price = new BigDecimal(priceText);
							option.setOptionPrice(price);
						} catch(Exception e) {
							throw new IntegrationExceptionService("Can't convert to numeric price " + priceText);
						}
					}
				}
			}
/*			if (options != null) {
			...
			throw new IntegrationExceptionService(e1);
		} finally {
			if (reader != null) {
				try {
					reader.close();
