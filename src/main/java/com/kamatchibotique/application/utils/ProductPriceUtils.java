...
//		Validate.notNull(variant.getAuditSection(), "variant.availabilities must not be null or empty");
    ...
    public String getFormatedAmountWithCurrency(Locale locale,
                                                com.kamatchibotique.application.model.reference.currency.Currency currency, BigDecimal amount) throws Exception {
        if (amount == null) {
            return "";
        }
     ...
     */
    // Utility
    public String getFormatedAmountWithCurrency(com.kamatchibotique.application.model.reference.currency.Currency currency,
    ...
    private void discountPrice(FinalPrice finalPrice) {
        ...
        // Using BigDecimal for precise arithmetic operations
        BigDecimal specialAmount = finalPrice.getProductPrice().getProductPriceSpecialAmount();
        BigDecimal originalAmount = finalPrice.getProductPrice().getProductPriceAmount();

        // Calculate the arithmetic for discount percentage
        if (originalAmount.compareTo(BigDecimal.ZERO) > 0) {
            BigDecimal arith = specialAmount.divide(originalAmount, 4, RoundingMode.HALF_UP);
            BigDecimal fsdiscount = BigDecimal.valueOf(100).subtract(arith.multiply(BigDecimal.valueOf(100)));

            // Convert fsdiscount to float
            Float percentageDiscount = fsdiscount.floatValue();
            int percent = percentageDiscount.intValue();

            // Set the discount percent
            finalPrice.setDiscountPercent(percent);
        } else {
            // Handle the case where the original amount is zero to avoid division by zero
            finalPrice.setDiscountPercent(0);
        }
